...
class StepType(str, Enum):
    ...
    `agent` is a free-form display name that differs per method ("Tool Generator",
    "assistant", "WorkerAgent", ...), so anything reading the trace would have to
    parse those strings and special-case every method. StepType is the stable
    handle instead: an explanation is built from what a step *did*, not from what
    the method happens to call it.
    """
    ROUTING = "routing"        # deciding which agent/trio handles the request
    PLAN = "plan"              # decomposing the request into subtasks
    TOOL_CALL = "tool_call"    # producing tool calls
    CORRECTION = "correction"  # regenerating tool calls after a validation error
    EVALUATE = "evaluate"      # judging whether results suffice / whether to continue
    OUTPUT = "output"          # writing the answer shown to the user
    ERROR = "error"
...
class AgentMessage(BaseModel):
    ...
    Attributes:
        ...
        step_type: The semantic role of this step; see StepType
        model: The model that produced this step, e.g. "openai/gpt-4o-mini"
        iteration: Which internal round this step belongs to, counting from 0
        parent_id: Id of the step that spawned this one, for methods that nest

    The last four exist for explainability. `parent_id` matters most in
    self-orchestrated, where worker steps are appended from inside
    asyncio.gather: list order there does not reflect the logical structure, so
    a reader without a parent link would reconstruct a sequence that never
    happened. All four are optional so that sessions stored before this field
...
class QueryResponse(BaseModel):
    ...
    Attributes:
        ...
        response_id: Stable id for this response, so an explanation can be
            addressed to it. Chats are addressed by list index elsewhere, but an
            index shifts if a response is ever inserted or removed, and an
...
class ToolCall(BaseModel):
    ...
    Attributes:
        success: Whether the invocation succeeded. None means "not invoked yet",
            which is a distinct state from failure: a call awaiting user approval
            has no outcome. Errors currently live inside `result` as a string,
            so a reader cannot otherwise tell a failure from a result that merely
            reads like one.
    ...
    def without_id(self):
        ...
        Bookkeeping fields are dropped along with the ID. They carry no
        information the model does not already have -- a failure is visible in
        `result` -- and adding fields to what the model sees would change how
        methods behave, which this must not do.
        """
        return {k: v for k, v in self.model_dump().items() if k not in self._LLM_HIDDEN_FIELDS}
